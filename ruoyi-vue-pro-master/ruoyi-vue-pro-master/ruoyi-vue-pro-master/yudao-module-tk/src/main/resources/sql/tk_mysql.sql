...
CREATE TABLE IF NOT EXISTS `tk_generation_task` (
  ...
  `script_title` varchar(255) DEFAULT NULL COMMENT '生成时选择的文案标题',
  ...
  `title` varchar(128) DEFAULT NULL COMMENT '用户自定义生成任务名称',
  `creator` varchar(64) DEFAULT '' COMMENT '创建者',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updater` varchar(64) DEFAULT '' COMMENT '更新者',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
VALUES
  ...
  (1, 'TIKTOK', 'default-scopes', 'user.info.basic,user.info.profile,user.info.stats,video.publish,video.upload,video.list', '默认授权范围', 0),
