...
@Import({AdminUserServiceImpl.class, TkTenantAccessService.class})
public class AdminUserServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void testCreateUser_assignDefaultTenantUserRole() {
        AdminUserDO tenantAdmin = randomAdminUserDO(o -> {
            o.setId(10L);
            o.setUsername("tenantadmin");
            o.setTenantId(100L);
            o.setTkUserLevel("TENANT_ADMIN");
        });
        userMapper.insert(tenantAdmin);
        UserSaveReqVO reqVO = new UserSaveReqVO();
        reqVO.setUsername("tenantuser");
        reqVO.setNickname("Tenant User");
        reqVO.setPassword("123456");
        reqVO.setTkUserLevel("TENANT_USER");
        ...
        LoginUser loginUser = new LoginUser().setId(10L);

        try (MockedStatic<SecurityFrameworkUtils> security = mockStatic(SecurityFrameworkUtils.class)) {
            security.when(SecurityFrameworkUtils::getLoginUser).thenReturn(loginUser);
            Long userId = userService.createUser(reqVO);

            RoleDO role = roleMapper.selectByCode("tenant_user");
            assertNotNull(role);
            assertEquals("普通用户", role.getName());
            assertEquals(RoleTypeEnum.SYSTEM.getType(), role.getType());
            verify(permissionService).assignUserRole(eq(userId), eq(singleton(role.getId())));
        }
    }
    ...
    @Test
    public void testCreateUser_platformAdminRequiresTargetTenant() {
        AdminUserDO platformAdmin = randomAdminUserDO(o -> {
            o.setId(1L);
            o.setUsername("platformadmin");
            o.setTenantId(0L);
            o.setTkUserLevel("PLATFORM_ADMIN");
        });
        ...
        loginUser.setInfo(Collections.singletonMap("tkUserLevel", "PLATFORM_ADMIN"));

        try (MockedStatic<SecurityFrameworkUtils> security = mockStatic(SecurityFrameworkUtils.class)) {
            security.when(SecurityFrameworkUtils::getLoginUser).thenReturn(loginUser);
            assertServiceException(() -> userService.createUser(reqVO), USER_TK_TENANT_REQUIRED);
        }
    }
    ...
    @Test
    public void testGetUserPage_filterByTenantId() {
        AdminUserDO dbUser = randomPojo(AdminUserDO.class, o -> {
            o.setUsername("tenant-admin");
            o.setTenantId(100L);
        ...
        UserPageReqVO reqVO = new UserPageReqVO();
        reqVO.setTenantId(100L);

        PageResult<AdminUserDO> pageResult = userService.getUserPage(reqVO);

        assertEquals(1, pageResult.getTotal());
        assertEquals(1, pageResult.getList().size());
        assertEquals(dbUser.getId(), pageResult.getList().get(0).getId());
    }
