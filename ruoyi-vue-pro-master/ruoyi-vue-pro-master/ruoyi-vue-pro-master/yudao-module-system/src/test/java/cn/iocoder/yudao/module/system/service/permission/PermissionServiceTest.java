...
@Import({PermissionServiceImpl.class, TkTenantAccessService.class})
public class PermissionServiceTest extends BaseDbUnitTest {
    ...
    @Test
    public void testAssignUserRole_rejectsCrossTenantBinding() {
        Long userId = 1L;
        Set<Long> roleIds = asSet(200L);
        AdminUserDO user = new AdminUserDO().setId(userId);
        user.setTenantId(0L);
        RoleDO role = new RoleDO().setId(200L)
                .setStatus(CommonStatusEnum.ENABLE.getStatus());
        role.setTenantId(166L);
        when(userService.getUser(userId)).thenReturn(user);
        when(roleService.getRoleList(roleIds)).thenReturn(singletonList(role));

        ServiceException exception = assertThrows(ServiceException.class,
                () -> permissionService.assignUserRole(userId, roleIds));

        assertTrue(exception.getMessage().contains("用户与角色所属租户不一致"));
        assertTrue(userRoleMapper.selectList().isEmpty());
    }
