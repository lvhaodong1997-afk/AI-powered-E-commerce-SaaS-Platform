...
@Import({RoleServiceImpl.class, TkTenantAccessService.class})
public class RoleServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void testCreateRole_platformAdminRequiresTargetTenant() {
        AdminUserDO platformAdmin = randomPojo(AdminUserDO.class, o -> {
            o.setId(1L);
            o.setUsername("platformadmin");
            o.setPassword("encoded");
            o.setNickname("Platform Admin");
            o.setSex(0);
        ...
        try (MockedStatic<SecurityFrameworkUtils> security = mockStatic(SecurityFrameworkUtils.class)) {
            ...
            assertServiceException(() -> roleService.createRole(reqVO, null), USER_TK_TENANT_REQUIRED);
        }
    }
    ...
    public void testUpdateRole() {
        ...
        assertPojoEquals(reqVO, newRoleDO, "tenantId");
        assertEquals(roleDO.getTenantId(), newRoleDO.getTenantId());
    }
    ...
    @Test
    public void testGetRolePage_excludeTenantAdmin() {
        RoleDO customRole = randomPojo(RoleDO.class, o -> {
            o.setName("运营");
            o.setCode("operator");
            o.setType(RoleTypeEnum.CUSTOM.getType());
        });
        roleMapper.insert(customRole);
        RoleDO tenantAdminRole = randomPojo(RoleDO.class, o -> {
            o.setName("租户管理员");
            o.setCode("tenant_admin");
            o.setType(RoleTypeEnum.SYSTEM.getType());
        });
        roleMapper.insert(tenantAdminRole);

        RolePageReqVO reqVO = new RolePageReqVO();
        reqVO.setIncludeTenantAdmin(false);

        PageResult<RoleDO> pageResult = roleService.getRolePage(reqVO);

        assertEquals(1, pageResult.getTotal());
        assertEquals(1, pageResult.getList().size());
        assertEquals(customRole.getId(), pageResult.getList().get(0).getId());
    }
