...
public class RoleServiceImpl implements RoleService {
    ...
    public Long createRole(RoleSaveReqVO createReqVO, Integer type) {
        return tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> {
            ...
            if (tkTenantAccessService.isCurrentUserPlatformAdmin()
                    && (role.getTenantId() == null || role.getTenantId() <= 0)) {
                throw exception(USER_TK_TENANT_REQUIRED);
            }
            tkTenantAccessService.applyCurrentTenantForNonPlatform(role);
    ...
    public void updateRole(RoleSaveReqVO updateReqVO) {
        tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> {
            ...
            tkTenantAccessService.validateRoleTenant(role);
            // 1.2 校验角色的唯一字段是否重复
            ...
            updateObj.setTenantId(role.getTenantId());
    ...
    public void deleteRoleList(List<Long> ids) {
        tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> {
            // 1. 校验是否可以删除
            ids.forEach(id -> tkTenantAccessService.validateRoleTenant(validateRoleForUpdate(id)));
    ...
    public List<RoleDO> getRoleListByStatus(Collection<Integer> statuses) {
        return tkTenantAccessService.executeCurrentTenantForNonPlatform(() ->
                roleMapper.selectListByStatus(statuses, getForcedTenantId()));
    }

    @Override
    public List<RoleDO> getRoleList() {
        return tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> {
            Long tenantId = getForcedTenantId();
            return tenantId == null ? roleMapper.selectList()
                    : roleMapper.selectList(RoleDO::getTenantId, tenantId);
        });
    }
    ...
    public List<RoleDO> getRoleList(Collection<Long> ids) {
        ...
        return tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> roleMapper.selectByIds(ids));
    }
    ...
    public PageResult<RoleDO> getRolePage(RolePageReqVO reqVO) {
        return tkTenantAccessService.executeCurrentTenantForNonPlatform(() ->
                roleMapper.selectPageByTenant(reqVO, getRequestedTenantId(reqVO.getTenantId())));
    }
