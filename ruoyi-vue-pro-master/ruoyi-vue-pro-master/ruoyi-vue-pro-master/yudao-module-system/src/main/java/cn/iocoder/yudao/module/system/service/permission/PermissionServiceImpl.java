...
public class PermissionServiceImpl implements PermissionService {
    ...
    public void assignRoleMenu(Long roleId, Set<Long> menuIds) {
        tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> {
            ...
            if (CollUtil.isNotEmpty(createMenuIds)) {
                ...
                tkTenantAccessService.applyCurrentTenantForNonPlatform(entity);
                return entity;
            }));
            }
    ...
    public void assignUserRole(Long userId, Set<Long> roleIds) {
        tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> {
            cn.iocoder.yudao.module.system.dal.dataobject.user.AdminUserDO user = userService.getUser(userId);
            roleService.validateRoleList(roleIds);
            List<RoleDO> roles = roleService.getRoleList(roleIds);
            if (CollUtil.isNotEmpty(roles) && roles.stream()
                    .anyMatch(role -> !Objects.equals(user.getTenantId(), role.getTenantId()))) {
    ...
    public Set<Long> getUserRoleIdListByUserId(Long userId) {
        return tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> {
            ...
            Set<Long> roleIds = convertSet(userRoleMapper.selectListByUserId(userId), UserRoleDO::getRoleId);
            if (CollUtil.isEmpty(roleIds) || tkTenantAccessService.isCurrentUserPlatformAdmin()) {
                return roleIds;
            }
            return convertSet(roleService.getRoleList(roleIds), RoleDO::getId);
        });
    }
    ...
    public Set<Long> getUserRoleIdListByRoleId(Collection<Long> roleIds) {
        return tkTenantAccessService.executeCurrentTenantForNonPlatform(() -> {
            ...
            if (CollUtil.isEmpty(userIds) || tkTenantAccessService.isCurrentUserPlatformAdmin()) {
                return userIds;
            }
            return convertSet(userService.getUserList(userIds), user -> user.getId());
        });
    }
