...
import { translateTkTitle } from '@/utils/tkI18n'
...
const treeProps = { ...defaultProps, label: 'displayName' }
type DeptTree = Tree & { displayName?: string; children?: DeptTree[] }
const filterText = ref('') // 过滤关键字
const deptList = ref<DeptTree[]>([]) // 部门树数据
const treeRef = ref<InstanceType<typeof ElTree>>() // 树 Ref
let currentNodeId: number | null = null // 当前选中的节点 ID

const translateDeptTree = (nodes: Tree[]): DeptTree[] =>
  nodes.map((node) => ({
    ...node,
    displayName: translateTkTitle(node.name),
    children: node.children ? translateDeptTree(node.children) : undefined
  }))

/** 加载部门树 */
const loadTree = async () => {
  const res = await DeptApi.getSimpleDeptList()
  deptList.value = translateDeptTree(handleTree(res))
}

/** 基于名字过滤 */
const filterNode = (name: string, data: DeptTree) => {
  if (!name) return true
  return data.name.includes(name) || Boolean(data.displayName?.includes(name))
}
...
watch(
  () => translateTkTitle('总公司'),
  () => {
    deptList.value = translateDeptTree(deptList.value)
  }
)
</script>
