...
const dataScopeLabels: Record<number, string> = {
  [SystemDataScopeEnum.ALL]: '全部数据权限',
  [SystemDataScopeEnum.DEPT_CUSTOM]: '指定部门数据权限',
  [SystemDataScopeEnum.DEPT_ONLY]: '部门数据权限',
  [SystemDataScopeEnum.DEPT_AND_CHILD]: '部门及以下数据权限',
  [SystemDataScopeEnum.DEPT_SELF]: '仅本人数据权限'
}
const dataScopeOptions = computed(() =>
  getIntDictOptions(DICT_TYPE.SYSTEM_DATA_SCOPE).map((item) => ({
    ...item,
    label: dataScopeLabels[item.value] || item.label
  }))
)
