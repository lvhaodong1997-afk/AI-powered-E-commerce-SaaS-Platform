<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="内置角色" prop="includeTenantAdmin">
        <el-switch
          v-model="queryParams.includeTenantAdmin"
          active-text="显示租户管理员"
          inactive-text="默认隐藏"
        />
      </el-form-item>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" @selection-change="handleRowCheckboxChange">
      <el-table-column type="selection" width="55" :selectable="canSelectRole" />
      ...
      <el-table-column align="center" label="所属租户" prop="tenantId" width="180">
        <template #default="scope">
      ...
      <el-table-column :width="300" align="center" label="操作">
        <template #default="scope">
          ...
          <el-button
            v-hasPermi="['system:role:delete']"
            :disabled="isTenantAdminRole(scope.row)"
            link
            type="danger"
...
const getTenantName = (tenantId?: number) => {
  ...
  return tenantList.value.find((tenant) => tenant.id === tenantId)?.name || `租户 ${tenantId}`
}

const isTenantAdminRole = (row: RoleApi.RoleVO) => row.code === 'tenant_admin'
const canSelectRole = (row: RoleApi.RoleVO) => !isTenantAdminRole(row)
