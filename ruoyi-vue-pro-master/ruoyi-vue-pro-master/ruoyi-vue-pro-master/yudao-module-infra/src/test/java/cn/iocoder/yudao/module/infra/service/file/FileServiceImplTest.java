...
public class FileServiceImplTest extends BaseDbUnitTest {
    ...
    public void testDeleteFileByUrl_success() {
        ...
        // 调用
        fileService.deleteFileByUrl(url + "?sign=temporary");

        // 校验数据不存在了
        assertNull(fileMapper.selectById(dbFile.getId()));
        // 校验调用
        verify(client).delete(eq("tk/material/demo.mp4"));
    }

    @Test
    public void testDeleteFileByUrl_blankUrlIgnored() {
        // 调用
        fileService.deleteFileByUrl(" ");
    ...
    @Test
    public void testDeleteFileByUrl_notExistsIgnored() {
        // 调用
        fileService.deleteFileByUrl("https://cdn.example.com/tk/material/missing.mp4");
